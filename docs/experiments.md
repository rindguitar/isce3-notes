...
* → **2 次元の実データはまだ存在しない。**門は、まだ無いケースのために
  現存する唯一のケースを閉ざしている
...
→ **B は将来 2 次元化したときの動きを変えず**（④ と ② が完全一致）、
**保存形式によらず同じ答えを出す**（③ も ② と完全一致、いずれも値の最大差 0.0）。
...
### `or` 版での再検証（2026-09-23 / 2026-09-27。notes PR #2 のレビューを受けて）

条件式を **`if not flag_luts_are_1d_az or slant_range_path in self.input_hdf5_obj:`** に変えた。
旧案（`if slant_range_path in ...`）では `skip_if_not_present` の失敗経路に届かず、
軸の無い 2 次元 LUT が合成軸で黙って処理されてしまうため（レビューの指摘）。

- **2×2 は数字が同じ**（2026-09-23、`0c775d7ae`）: 1 次元 0/410 → 178/410、
  2 次元は 178/410 のまま。修正後の 2 通りはどちらも「2 次元・未修正」とマスク一致・最大差 0.0
- **ctest の gcov / gslc でも成立**（2026-09-27、`0c775d7ae`）
  - GDAL のエラー: gcov 4 → 0、gslc 2 → 0（**計 6 → 0**）
  - `(rg. vector)` は **4 行のまま**。4 行の正体は crosstalk の 4 データセット
    （tx/rx × Horizontal/Vertical Crosspol）で、**実行回数ではない**
  - `(az. vector)` は 0 → **1 行**（1 次元の経路に入るようになった）
  - ⚠️ 行数が実行回数（gcov は 4 回）と合わないのは、Python の `warnings.warn` が
    **同じ文面を 1 回しか表示しない**ため
  - **出力 6 本すべてで `grids/` 以下は全データセットがビット単位で一致**
    （GCOV 11/11 × 4 本、GSLC 13/13 × 2 本）。`grids/` 以外で変わったのは
    **`referenceTerrainHeight`（0 → 178/410）と `identification/processingDateTime`（実行時刻）だけ**
- **`ctest` の `-V` は必須**（2026-09-23）: `-V` 無しは 7 行・エラー 0 回、あり 3473 行・4 回。
  `--output-on-failure` は**合格したテストの出力を出さない**
- 実行はインストール先の複製にパッチを当てて行い、**md5 で復元を確認**
  （`c357fd5b…`）。テスト出力も修正前のものに戻した

### 実データのエラー文はどこで見たものか（2026-09-27 確認）

`Requested (0,0) of size 105x31 on raster of 31x1` は、**配布物から出たものではない**。
2026-08-30 に **028_168 の RSLC を手元で再処理したとき**のログ
（`logs/2026-08-30-first-real-data.md`）。028_168 の RSLC は
`zeroDopplerTime (31,)` / `slantRange (105,)` で数字が一致する。
**028_152 はまだ再処理していない**ので、そちらのエラー文は見ていない。
...
詳細はこの節と §5（`referenceTerrainHeight` が全 NaN になる）にまとめてある。
