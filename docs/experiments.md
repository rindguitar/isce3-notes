...
### 修正案を当てた結果（`~/isce3` は書き換えず複製で検証）

判定を「兄弟に `slantRange` があるか」→「**そのデータセット自身が 1 次元か**」に変更。

| | 修正前 | 修正後 |
|---|---|---|
| GCOV / GSLC テストのエラー | 4 回 / あり | **0 回 / 0 回** |
| `referenceTerrainHeight`（envisat） | 0/410 | **134/410** |
| `referenceTerrainHeight`（実データ） | 0/156,420 | **31,228/156,420** |
| テストの合否 | 合格 | **合格**（変わらず） |
