...
**このファイルは薄く保つこと。** 詳細は `docs/` と `.claude/rules/`、
そして **wiki の用語集**に置き、必要時のみ参照する（末尾の「参照ドキュメント」）。
...
ISCE3 = NASA JPL の InSAR / SAR 処理ライブラリ。C++/CUDA のコア + Python
バインディング。NISAR ミッションの処理基盤。扱うリポジトリは 2 つ。
...
### 🔴 最重要: `~/isce3` は「読むのは自由・書くのは確認必須」

**読み取り: 全面的に許可されている**（2026-08-28 に変更）。
ソースコード全体を許可を求めずに読んでよい。`cxx/`・`python/`・`tests/`・
`share/`・`.github/` など、**例外なく全部**。

**書き換え: 毎回ユーザーの確認が要る。** 対象ファイルと変更内容を述べ、
承認を得てから編集する。承認の範囲を次のファイルへ推測で広げない。

| 操作 | 扱い |
|---|---|
| 読む・grep する・ディレクトリを辿る | **許可不要** |
| 新規ファイルを作る / 既存を編集する | **毎回確認する** |
| `git` の書き込み系（commit / checkout / branch 作成） | **毎回確認する** |

読むのが自由になったぶん、**書いた内容の裏取りをソースで行う義務が生じた。**
分野の一般知識で済ませず、実装を確認して「確認済み」と書けるようにする。
...
* **ctest の基準値は 236/237**（2026-08-30 に更新）。`nisar.workflows.stage_dem` のみ
  既知の失敗。**2 件以上落ちたら自分の変更を疑う**

## セッションプロトコル（最重要）

### 開始時

1. `docs/STATUS.md`（このファイルに @import 済み）で現在地・次の一手・未解決事項を把握する
2. 作業を始める前に**今日のゴールを 1〜3 行で確認**し、ユーザーと認識を合わせる
3. `~/isce3` で作業するなら `git -C ~/isce3 branch --show-current` で現在のブランチを確認する

### 作業完了時（1 タスク終わるごと。セッション終了時にまとめてではない）

**`/clear` の前もここに含まれる。** 会話の記憶が消えるので、
`docs/STATUS.md` に残していないことは失われる。ユーザーが `/clear` すると言ったら、
その前に下記を済ませたか確認する。
...
* IMPORTANT: `logs/` `reference/` `drafts/` には `README.md` を必ず置き、
  **中身を変更したのと同じコミットで更新する**（詳細は `.claude/rules/readme.md`）
* **構造・流れ・依存関係の説明は Mermaid 図にする。** 文章だけで済ませない
  （処理の順序、パスの関係、状態遷移など）。作図原則は `.claude/rules/mermaid.md`
* **PNG はコミットしない。** ` ```mermaid ` フェンスで md に直接埋める
* 概念そのものの解説は**リポジトリではなく wiki**へ書く（`.claude/rules/wiki.md`）
* **推測で書かない。** `~/isce3` は自由に読めるので、事実はソースで裏を取ってから書く
...
* サブエージェントも `~/isce3` を自由に読んでよい。ただし**書き換えは同じく確認必須**。
  委譲するときは「読み取りのみ」と明示する
...
2. 承認後に実装
3. ビルド → テスト → **基準値 236/237 と比較** → clang-format → ドキュメント更新 →
   コミット → プッシュ（**この順序を崩さない**）
...
**このプロジェクトに `Makefile` は無い。** ビルドは CMake を直接叩く。
...
構成をやり直す場合のオプションは `reference/build-options.md` を参照。

⚠️ ctest の `DEPENDS` は順序を決めるだけで前提テストを自動実行しない。
`-R` で絞ると前提が走らず落ちるものがある。
...
* コミットメッセージは**英語**。upstream の既存の書き方に合わせる（接頭辞は付けない）

### `~/isce3-notes`（このリポジトリ）

* **`main` へ直接コミットしてよい**（このリポジトリのみの例外）。
  記録用なので、`docs/` `logs/` `reference/` の更新はブランチを切らない
* 🔴 **PR は「メンターに approve をもらう」ためだけに使う。**
...
* `drafts/` は提出前の下書き。提出したら状態を `drafts/README.md` に反映する
* wiki は別リポジトリ。編集するには
  `git clone https://github.com/rindguitar/isce3-notes.wiki.git` が要る

### ブランチの寿命

* 長寿命ブランチ（マージ後も削除しない）は以下の**のみ**:
  `~/isce3` の `develop` と、`~/isce3-notes` の `main`
* リストにないブランチはすべて短命な作業ブランチ。**マージ済みになったら
  ローカル・リモートとも削除する**（未マージ = 使用中の印として運用）
* マージ後に同じテーマで作業を続ける場合も、古いブランチには戻らず新しく切り直す

## セキュリティ

* **このリポジトリは公開されている。** 資格情報・トークンの中身を書かない
  （Earthdata のユーザトークンは `~/.edl_token`、権限 600。**中身をログに貼らない**）
* パスは `~/...` で書く。**ユーザ名を含む絶対パスを書かない**
* `~/isce3` のコミットは detect-secrets と CodeQL でスキャンされる。
  秘密情報を含めない。疑わしければコミットしない
* 実データ（`~/nisar-data/`）は**どちらのリポジトリにも入れない**
...
| `drafts/*.md` | 提出前の下書き | issue / PR を出すとき |
