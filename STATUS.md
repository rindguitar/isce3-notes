...
**環境構築 → 実データ取得 → 公式プロダクトの再現 完了 → 🔴 バグを 1 件発見。issue 起票の準備へ。**
...
1. 🔴 **`referenceTerrainHeight` のジオコーディング失敗を issue にする**（最優先）
   - 再現済み・原因特定済み・既知 issue と別物であることも確認済み
   - **修正案は未検証。** `var_h5_dataset.ndim` で判定する案を試してから出すか、
     先に issue を立てて反応を見るかは要判断
   - GSLC も同じ基底クラスを使うので、影響範囲の確認が要る
   - 進め方: **まず issue を立てて再現手順を示す**（PR はレビュー待ちが数か月のため）
...
- 🔴 **`referenceTerrainHeight` のジオコーディングが全 NaN になる**（2026-08-30 発見）。
  終了コード 0 で完了するため見落としやすい。issue 未起票。修正案も未検証
