...
**この文書は調査した順に並んでいる**（環境を疑うところから始まる）。
**人に説明する順序は逆**なので、話を組み立てるときは下の ①〜⑥ の順に拾う。
説明用に並べ替えたものが `issue-reference-terrain-height-説明.md`。
...
3. **テストコードに `referenceTerrainHeight` の出現が 0 回**（→ 検査するテストが存在しない）。
   ⚠️ `.h5` フィクスチャには**文字列として 11 件**ある。**データはあるのに検査だけが無い**
...
**追加された条件は「1 次元の判定」ではなく、「1 次元の道の入口に置いた門」だったと読める。**
⚠️ **止めているのは 1 次元の道への入口**（2 次元の道ではない）。構造は説明メモの図を参照。
...
⚠️ **この読み方は修正案 B と整合する。** B は「門の判定材料」と
